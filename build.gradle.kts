...
kotlin {
    ...
    swiftExport {
        moduleName = "Logos"
        flattenPackage = "io.github.kotlinmania.logos"
    }

    android {
        namespace = "io.github.kotlinmania.logos"
        compileSdk = 34
        minSdk = 24
...
//
// If commonMain has no .kt files (repo is in early parity), the task writes a
// tiny placeholder under `build/generated/codeql-empty-source/` so kotlinc
// always has at least one source. Skipping this task is intentionally avoided.
...
val codeqlCompileJvm = tasks.register<JavaExec>("codeqlCompileJvm") {
    description =
        "Compile commonMain Kotlin sources with kotlinc 2.3.21 for CodeQL Java/Kotlin extraction. " +
            ...
            "or `github/codeql-action/init` so the Java/Kotlin extractor can attach to the in-process " +
            "`org.jetbrains.kotlin.cli.jvm.K2JVMCompiler` invocation."
    group = "verification"
...
mavenPublishing {
    ...
    coordinates(group.toString(), "logos", version.toString())
    ...
    pom {
        ...
        scm {
            url.set("https://github.com/KotlinMania/logos-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/logos-kotlin.git")
